...
export const seed5000EmployeesToAtlas = async () => {
  try {
    ...
    let pAtlas: any = await Project.findOne({ code: 'PROJ-ATLAS' });
    if (!pAtlas) {
      pAtlas = await Project.create({ name: 'Project Atlas AI Core', code: 'PROJ-ATLAS', description: 'AI Core System', status: 'active' });
    }
    let pBeacon: any = await Project.findOne({ code: 'PROJ-BEACON' });
    if (!pBeacon) {
      pBeacon = await Project.create({ name: 'Project Beacon Analytics', code: 'PROJ-BEACON', description: 'Analytics Engine', status: 'active' });
    }
    let pNexus: any = await Project.findOne({ code: 'PROJ-NEXUS' });
    if (!pNexus) {
      pNexus = await Project.create({ name: 'Project Nexus Cloud', code: 'PROJ-NEXUS', description: 'Cloud Infrastructure', status: 'active' });
    }

    const projectsList = [pAtlas._id, pBeacon._id, pNexus._id, null];

    // Ensure Floors and Seats exist
    let seats: any[] = await Seat.find();
    if (seats.length === 0) {
      const floors: any[] = [];
      for (let f = 1; f <= 5; f++) {
        const fl = await Floor.create({ floorNumber: f, name: `Floor ${f}`, building: 'Ethara HQ' });
        floors.push(fl);
      }
      const zones: any[] = [];
      for (const fl of floors) {
        const zA = await Zone.create({ floorId: fl._id, zoneName: 'Zone A', capacity: 250 });
        const zB = await Zone.create({ floorId: fl._id, zoneName: 'Zone B', capacity: 250 });
        zones.push(zA, zB);
      }
    ...
    for (let b = 0; b < Math.ceil(totalToInsert / batchSize); b++) {
      ...
      if (employeesBatch.length > 0) {
        await Employee.insertMany(employeesBatch, { ordered: false }).catch(() => {});
