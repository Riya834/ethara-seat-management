...
const seedDatabase = async () => {
  try {
    ...
    for (const fl of floors) {
      const zA = await Zone.create({ floorId: fl._id, zoneName: 'Zone A - East Wing', capacity: 120 });
      const zB = await Zone.create({ floorId: fl._id, zoneName: 'Zone B - Central Hub', capacity: 120 });
      const zC = await Zone.create({ floorId: fl._id, zoneName: 'Zone C - West Wing', capacity: 120 });
      zonesList.push(zA, zB, zC);
    }

    // 3. Create Projects
    const projectAtlas = await Project.create({ name: 'Project Atlas AI Core', code: 'PROJ-ATLAS', description: 'Next-gen enterprise search and AI knowledge engine.', status: 'active', startDate: new Date('2025-10-01') });
    const projectBeacon = await Project.create({ name: 'Project Beacon Analytics', code: 'PROJ-BEACON', description: 'Real-time telemetry and executive data dashboard.', status: 'active', startDate: new Date('2025-11-15') });
    const projectNexus = await Project.create({ name: 'Project Nexus Cloud', code: 'PROJ-NEXUS', description: 'Multi-cloud infrastructure modernization initiative.', status: 'active', startDate: new Date('2026-01-05') });
    const projectOrion = await Project.create({ name: 'Project Orion Security', code: 'PROJ-ORION', description: 'Zero-trust network security and identity portal.', status: 'active', startDate: new Date('2026-02-01') });
    const projectPulse = await Project.create({ name: 'Project Pulse CRM', code: 'PROJ-PULSE', description: 'Global client relationship platform.', status: 'active', startDate: new Date('2026-03-01') });
    ...
    for (const z of zonesList) {
      ...
      const flNumber = (floors.find((f) => f._id.equals(z.floorId)) as any).floorNumber;

      for (let i = 1; i <= 115; i++) {
        const seatNumStr = `F${flNumber}-${zoneCode}-${String(i).padStart(3, '0')}`;
        let tag = null;
        if (flNumber === 2 && zoneCode === 'ZA') tag = projectAtlas._id;
        if (flNumber === 3 && zoneCode === 'ZB') tag = projectBeacon._id;
    ...
    const adminUser = await User.create({ name: 'System Admin', email: 'admin@ethara.com', passwordHash: defaultPasswordHash, role: 'admin', employeeId: adminEmp._id });
    ...
    await User.create({ name: 'Alex Project Manager', email: 'pm.atlas@ethara.com', passwordHash: defaultPasswordHash, role: 'pm', employeeId: pmEmp._id });

    const empJohn = await Employee.create({
    ...
    await User.create({ name: 'John Doe', email: 'emp.john@ethara.com', passwordHash: defaultPasswordHash, role: 'employee', employeeId: empJohn._id });
    ...
    for (let i = 5; i <= 5000; i++) {
      ...
      let allocStatus: 'allocated' | 'pending' = isNewJoiner && i % 3 === 0 ? 'pending' : 'allocated';
    ...
    const insertedEmployees = await Employee.insertMany(employeeBatch);
